...
func (o *OpsCmp) BatchCreate(ctx context.Context, index string, docs []map[string]interface{}, isWithID bool) (err error) {
	defer func() {
		if err != nil {
			helpers.RecordErrLogWithPos(o.logger, err, "BatchCreate", "BatchCreate")
		}
	}()
	...
	for i, item := range docs {
		...
		dataJSON, err = utils.JSON().Marshal(item)
		if err != nil {
...
func (o *OpsCmp) BatchCreateInterface(ctx context.Context, index string, docs interface{}, isWithID bool, batchSize int) (err error) {
	defer func() {
		if err != nil {
			helpers.RecordErrLogWithPos(o.logger, err, "BatchCreateInterface", "BatchCreateInterface")
		}
	}()

	// 1. interface{} to []map[string]interface{}
	// 1.1 interface{} to []byte
	bys, err := utils.JSON().Marshal(docs)
	if err != nil {
	...
	err = utils.JSON().Unmarshal(bys, &maps)
	if err != nil {
