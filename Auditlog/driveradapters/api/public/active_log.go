...
var (
	...
	ac              interfaces.PublicRESTHandler
	avaliableParams *paramutils.AvailableParam
)

type activeLogHandler struct {
	activeLog interfaces.ActiveLog
}

// NewActiveLogHandler 创建document handler对象
func NewActiveLogHandler() interfaces.PublicRESTHandler {
	activeOnce.Do(func() {
		ach := &activeLogHandler{
...
func (ac *activeLogHandler) RegisterPublic(routerGroup *gin.RouterGroup) {
	getRoler := middleware.PermissionMiddleware([]string{common.SuperAdmin, common.SecAdmin, common.SysAdmin, common.AuditAdmin, common.OrgAudit})
	routerGroup.POST(
		"/report-center/active/:category/list",
		getRoler,
		middleware.VisitorParser,
		ac.getDataList,
	)

	getFieldValuesRoler := middleware.PermissionMiddleware([]string{common.SuperAdmin, common.SecAdmin, common.SysAdmin, common.AuditAdmin, common.OrgAudit})
	routerGroup.POST(
		"/report-center/active/:category/field/:field/values",
		getFieldValuesRoler,
		middleware.VisitorParser,
		ac.getFieldValues,
	)
...
func (ac *activeLogHandler) getDataList(c *gin.Context) {
	// 获取path中参数
...
func (ac *activeLogHandler) getFieldValues(c *gin.Context) {
	// 获取path中参数
	logType := c.Param("category")
	field := c.Param("field")
