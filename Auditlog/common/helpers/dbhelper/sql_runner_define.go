...
type SQLRunner struct {
	...
	sb            *sqlhelper.SelectBuilder
	deleteBuilder *sqlhelper.DeleteBuilder
	ib            *sqlhelper.InsertBuilder
	ub            *sqlhelper.UpdateBuilder
	db            ISQLRunner
	tag           string
	rawSQL        string // 原始sql
...
func NewSQLRunner(db ISQLRunner, logger api.Logger) *SQLRunner {
	tmp := &SQLRunner{
		logger: logger,
		db:     db,
...
func TxSr(tx *sql.Tx, logger api.Logger) *SQLRunner {
	return NewSQLRunner(tx, logger)
}

func SrByISQLRunner(db ISQLRunner, logger api.Logger) *SQLRunner {
	return NewSQLRunner(db, logger)
}

func NewQueryWithSQLBuilder(db ISQLRunner, sb *sqlhelper.SelectBuilder, logger api.Logger) *SQLRunner {
	tmp := &SQLRunner{
		logger: logger,
...
func (q *SQLRunner) updateBuilders() {
	q.deleteBuilder = sqlhelper.NewDeleteBuilder()
	q.deleteBuilder.SetWhereBuilder(q.sb.GetWhereBuilder())

	q.ub = sqlhelper.NewUpdateBuilder()
	q.ub.SetWhereBuilder(q.sb.GetWhereBuilder()).
		Tag(q.tag)

	q.ib = sqlhelper.NewInsertBuilder()
	q.ib.Tag(q.tag)
}

func (q *SQLRunner) SetSQLBuilder(sb *sqlhelper.SelectBuilder) *SQLRunner {
	q.sb = sb
	q.updateBuilders()
...
func (q *SQLRunner) struct2ScanArgsByTag(obj interface{}, opts ...Option) (args []interface{}) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if key, ok := t.Field(i).Tag.Lookup(q.tag); ok {
			...
			if len(selectFields) > 0 && !utils.Exists(selectFields, key) {
				continue
			}
