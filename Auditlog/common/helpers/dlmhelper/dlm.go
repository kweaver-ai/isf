...
import (
	...
	dbaulid "AuditLog/drivenadapters/db/ulid"
...
func genRedisDlmUniqueValue() (value string, err error) {
	if uniqueIDRepo == nil {
		uniqueIDRepo = dbaulid.NewUlidRepo(logcmp.GetLogger())
	}

	ctx := context.Background()
	value, err = uniqueIDRepo.GenUniqID(ctx, uniqidenums.UniqueIDFlagRedisDlm)
...
func delRedisDlmUniqueValue(value string) (err error) {
	...
	ctx := context.Background()
	err = uniqueIDRepo.DelUniqID(ctx, uniqidenums.UniqueIDFlagRedisDlm, value)
