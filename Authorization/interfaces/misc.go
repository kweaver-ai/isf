...
import (
	...
	mqSDK "github.com/kweaver-ai/proton-mq-sdk-go"
)

//go:generate mockgen -package mock -source ./misc.go -destination ./mock/misc.go

// MQClient 接口嵌入，为driveradapter/mq_client提供mock对象
type MQClient interface {
	mqSDK.ProtonMQClient
}

// DepHTTPSvc 接口嵌入，为drivenadapter下http接口提供mock对象
type DepHTTPSvc interface {
	// HandleRequest 处理函数
	HandleRequest(method, url string, reqBody any) (code int, resBody []byte)
}
...
type RedisConn interface {
	// Subscribe 消息订阅
	Subscribe(channel string, cmd func([]byte))

	// Publish 消息发布
	Publish(channel string, message []byte) (err error)
}
