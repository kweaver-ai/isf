...
<ValidateArea
    ...
    onValueChange={({detail}) => {
        setState({
            validateState: detail===''?'empty':/[#\\/:*?"<>|]/.test(detail)?'invalid':'normal',
            value: detail
        })
    }}
...
<ValidateArea
    ...
    onBlur={()=> setState({validateState:state.value===''?'empty':/[#\\/:*?"<>|]/.test(state.value)?'invalid':'normal'})}
    validateState={state.validateState}
    validateMessages={{
        ['empty']:'输入不允许为空',
        ['invalid']:'不允许输入非法字符',
...
<div>
    ...
    <div style={{ marginTop: '10px'}}>
        <Button onClick={() => {
            setState({
                validateState:state.value===''?'empty':/[#\\/:*?"<>|]/.test(state.value)?'invalid':'normal',
            })
        }}>{'确定'}</Button>
    </div>
</div>
