 ...
 */
export default function I18NFactory({ translations, locale = envLanguage() }: Settings) {
    ...
    const i18n: I18N = <I18N>function (resources = []) {
         ...
         */
        const transSourcesToDict = function (resources) {
        ...
        return function (key, replacements) {
            ...
            if (replacements) {
                return match.replace(/\${(.+?)}/g, (match, capture) => {
                    ...
                    return replacements[capture] !== undefined ? replacements[capture] : '';
                });
            } else {
                return match
            }
