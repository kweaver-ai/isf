...
import { Text, TextBox, UIIcon } from '@/ui/ui.desktop';
import { Switch, ValidateBox, NumberBox } from '@/sweet-ui';
import { ValidateMessage, Types, formatterTypeName, ConfigItem as Config } from '../helper';
        ...
        <div className={styles['container']}>
            {
                configItem.defaultConfig
                    ...
                    <div className={styles['item-name']}>
                        ...
                        <Text className={styles['name-content']}>{configItem.configName}</Text>
                    </div>
                    :
                    <div className={styles['item-name']}>
                        <div className={styles['tips']}>{__('名称：')}</div>
                        ...
                        <div className={styles['validate']}>
                            <ValidateBox
                                ...
                                onValueChange={({detail}) => onRequestConfigNameChange(detail)}
                            />
                        </div>
                    </div>
            }
            {
                configItem.defaultConfig || configItem.configType === Types.ObjectType
                    ?
                    <div className={styles['item-type']}>
                        <div className={styles['tips']}>{__('类型：')}</div>
                        <Text className={styles['type-content']}>{formatterTypeName(configItem.configType)}</Text>
                    </div>
                    :
                    <div className={styles['item-type']}>
                        <div className={styles['tips']}>{__('类型：')}</div>
            ...
            <div className={styles['item-value']}>
                ...
                <div className={styles['mark']}>{configItem.valueRequired ? '*' : ''}</div>
                {
                    configItem.configType === Types.StringType
                        ...
                        <div className={styles['validate']}>
                            <ValidateBox
                                ...
                                validateMessages={ValidateMessage}
                                onValueChange={({detail}) => onRequestConfigValueChange(detail, Types.StringType)}
                            />
                        </div>
                        :
