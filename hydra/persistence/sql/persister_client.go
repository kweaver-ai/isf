...
func (p *Persister) DeleteClient(ctx context.Context, id string) (err error) {
	...
	// 开始删除
	ctx, err = p.BeginTX(ctx)
	if err != nil {
		return err
	}
	defer func() {
		if err != nil {
			if rollBackTxnErr := p.Rollback(ctx); rollBackTxnErr != nil {
				err = rollBackTxnErr
			}
		}
	}()
	...
	// 删除flow表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&flow.Flow{})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除access表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&OAuth2RequestSQL{Table: sqlTableAccess})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除code表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&OAuth2RequestSQL{Table: sqlTableCode})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除hydra_oauth2_logout_request表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&consent.LogoutRequest{})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除hydra_oauth2_obfuscated_authentication_session表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&consent.ForcedObfuscatedLoginSession{})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除hydra_oauth2_oidc表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&OAuth2RequestSQL{Table: sqlTableOpenID})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除hydra_oauth2_pkce表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&OAuth2RequestSQL{Table: sqlTablePKCE})); err != nil {
		return sqlcon.HandleError(err)
	}

	// 删除hydra_oauth2_refresh表
	if err = sqlcon.HandleError(p.QueryWithNetwork(ctx).Where("client_id = ?", id).Delete(&OAuth2RequestSQL{Table: sqlTableRefresh})); err != nil {
		return sqlcon.HandleError(err)
	}
