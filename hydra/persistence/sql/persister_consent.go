...
func (p *Persister) RevokeSubjectLoginSession(ctx context.Context, subject string) error {
	...
	if err := p.QueryWithNetwork(ctx).Where("subject = ?", subject).First(&s); errors.Is(err, sql.ErrNoRows) {
		return nil
	} else if err != nil {
		return sqlcon.HandleError(err)
	}
	...
	return p.transaction(ctx, func(ctx context.Context, c *pop.Connection) error {
		nid := p.NetworkID(ctx)
		if err := c.RawQuery(
			"DELETE FROM hydra_oauth2_authentication_session WHERE nid = ? AND subject = ?",
			nid,
			subject,
		).Exec(); err != nil {
			return sqlcon.HandleError(err)
		...
		return sqlcon.HandleError(c.RawQuery(
			"UPDATE hydra_oauth2_flow SET login_session_id=NULL WHERE login_session_id=? AND nid = ?",
			s.ID,
			nid,
		).Exec())
	})
...
func (p *Persister) DeleteLoginSession(ctx context.Context, id string) error {
	...
	return p.transaction(ctx, func(ctx context.Context, c *pop.Connection) error {
		nid := p.NetworkID(ctx)
		count, err := c.RawQuery("DELETE FROM hydra_oauth2_authentication_session WHERE id=? AND nid = ?", id, nid).ExecWithCount()
		if count == 0 {
			return errorsx.WithStack(x.ErrNotFound)
		} else if err != nil {
			return sqlcon.HandleError(err)
