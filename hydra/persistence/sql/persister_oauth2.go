...
func (p *Persister) SetClientAssertionJWT(ctx context.Context, jti string, exp time.Time) error {
	...
	// 该接口内不再执行删除逻辑，详情参考文档: https://confluence.aishu.cn/pages/viewpage.action?pageId=222433167
...
func (p *Persister) flushInactiveTokens(ctx context.Context, notAfter time.Time, limit int, batchSize int, table tableName, lifespan time.Duration) error {
	...
	strSQL := fmt.Sprintf(`DELETE FROM %s WHERE signature in (
		SELECT signature FROM (SELECT signature FROM %s hoa WHERE requested_at < ? and nid = ? ORDER BY requested_at LIMIT ? ) as s
	)`, OAuth2RequestSQL{Table: table}.TableName(), OAuth2RequestSQL{Table: table}.TableName())
	dbType := os.Getenv("DB_TYPE")
	if strings.EqualFold(dbType, "MYSQL") || strings.EqualFold(dbType, "MARIADB") {
		strSQL = fmt.Sprintf(`DELETE a FROM %s AS a WHERE a.signature in (
			SELECT signature FROM (SELECT signature FROM %s hoa WHERE requested_at < ? and nid = ? ORDER BY requested_at LIMIT ? ) as s
		)`, OAuth2RequestSQL{Table: table}.TableName(), OAuth2RequestSQL{Table: table}.TableName())
	}
	...
	for deletedRecords := batchSize; totalDeletedCount < limit && deletedRecords == batchSize; {
		...
		deletedRecords, err = p.Connection(ctx).RawQuery(strSQL, notAfter, p.NetworkID(ctx), d).ExecWithCount()
...
func (p *Persister) flushInactiveAccessTokens(ctx context.Context, notAfter time.Time, limit int, batchSize int, table tableName, lifespan time.Duration) error {
	/* #nosec G201 table is static */
	// The value of notAfter should be the minimum between input parameter and token max expire based on its configured age
	requestMaxExpire := time.Now().Add(-lifespan)
	if requestMaxExpire.Before(notAfter) {
	...
	strSQL := fmt.Sprintf(`DELETE FROM %s WHERE signature in (
		SELECT signature FROM (SELECT a1.signature FROM %s as a1
		left join hydra_client as a2 on a1.client_id = a2.id 
		WHERE a1.requested_at < ? and a1.nid = ? 
		and (a1.client_id != a1.subject or a2.client_credentials_grant_access_token_lifespan is null)
		ORDER BY a1.requested_at LIMIT ? ) as s
	)`, OAuth2RequestSQL{Table: table}.TableName(), OAuth2RequestSQL{Table: table}.TableName())
	dbType := os.Getenv("DB_TYPE")
	if strings.EqualFold(dbType, "MYSQL") || strings.EqualFold(dbType, "MARIADB") {
		strSQL = fmt.Sprintf(`DELETE a FROM %s AS a WHERE a.signature in (
...
func (p *Persister) FlushInactiveAccessTokens(ctx context.Context, notAfter time.Time, limit int, batchSize int) error {
	return p.flushInactiveAccessTokens(ctx, notAfter, limit, batchSize, sqlTableAccess, p.config.GetAccessTokenLifespan(ctx))
}
