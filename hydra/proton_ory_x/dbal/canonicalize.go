...
const (
	...
	// DriverProtonRDS is the proton-rds driver name.
	DriverProtonRDS = "proton-rds"

	// UnknownDriver is the driver name if the driver is unknown.
	UnknownDriver = "unknown"
)

// Canonicalize returns constants DriverMySQL, DriverPostgreSQL, DriverCockroachDB, DriverProtonRDS, UnknownDriver, depending on `database`.
func Canonicalize(database string) string {
	...
	case "proton-rds":
		return DriverProtonRDS
	default:
		return UnknownDriver
	}
}

// MustCanonicalize returns constants DriverMySQL, DriverPostgreSQL, DriverCockroachDB, DriverProtonRDS or fatals.
